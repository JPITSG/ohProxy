...
describe('Input Surface Coverage', () => {
	it('tracks all req.query/req.body/req.params usages in server.js', () => {
		...
		const allowedPatterns = [
			...
			/const\s+\{\s*widgetId,\s*rules,\s*visibility,\s*visibilityUsers,\s*defaultMuted,\s*iframeHeight,\s*proxyCacheSeconds,\s*cardWidth,\s*cardWidthWidgetId\s*\}\s*=\s*req\.body\s*;/,
			...
			/const\s+rawQuerySitemap\s*=\s*typeof\s+req\.query\?\.sitemap\s*===\s*'string'\s*\?\s*req\.query\.sitemap\s*:\s*''\s*;/,
			/const\s+rawSitemapQuery\s*=\s*typeof\s+req\.query\?\.sitemap\s*===\s*'string'\s*\?\s*req\.query\.sitemap\s*:\s*''\s*;/,
			...
			/const\s+rawOffset\s*=\s*req\.query\?\.offset\s*;/,
			/const\s+rawVersion\s*=\s*req\.query\?\.v\s*;/,
...
describe('Input Validation Coverage', () => {
	it('validates POST body keys and values', () => {
		...
		assert.ok(content.includes("const allowedUserKeys = new Set(['trackGps', 'voiceModel', 'mapviewRendering', 'password', 'confirm']);"));
		...
		assert.ok(content.includes("errors.push('user.mapviewRendering must be \"ohproxy\" or \"openhab\"');"));
