...
describe('Presence zoom button state', () => {
	it('disables zoom controls at map zoom bounds using shared styling', () => {
		...
		assert.match(server, /function shouldDisableHomeButton\(\)\{\s*var target=getHomeTarget\(\);\s*if\(!target\)return true;\s*\/\/ A rotated map is never "already home" - the button also resets to north\.\s*if\(rotorTheta!==0\)return false;\s*var center=map\.getCenter\(\);\s*return !!\(center&&sameMapCenter\(center,target\.center\)&&map\.getZoom\(\)===target\.zoom\);/);
