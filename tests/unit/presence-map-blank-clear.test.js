...
describe('Presence map blank interaction clearing', () => {
	it('clears nearby days and pinned previous-point tooltip only from blank map clicks or taps', () => {
		...
		assert.match(server, /mapEl\.addEventListener\('touchend',function\(e\)\{[\s\S]*?if\(f\)\{[\s\S]*?showBlueAndHandleClick\(f,endPx,true\);[\s\S]*?\}else\{\s*clearPresenceMapPopupsFromBlankPixel\(endPx\);[\s\S]*?\}/);
