...
describe('Presence Nearby Days Anchor Follow', () => {
	it('repositions the nearby-days popup from stored map coordinates during map movement', () => {
		...
		assert.match(server, /function getCtxAnchorPixel\(\)\{[\s\S]*?if\(!isFinite\(ctxLat\)\|\|!isFinite\(ctxLon\)\)return null;[\s\S]*?var lonlat=new OpenLayers\.LonLat\(ctxLon,ctxLat\)\.transform\(wgs84,proj\);[\s\S]*?return mapPixelToRootPoint\(map\.getPixelFromLonLat\(lonlat\)\);[\s\S]*?\}/);
	...
	it('stores manual popup dragging as an offset from the anchored map point', () => {
		...
		assert.match(server, /function ctxUpdatePos\(e,persistOffset\)\{[\s\S]*?ctxMenuOffsetX=0;[\s\S]*?ctxMenuOffsetY=0;[\s\S]*?setCtxMenuPosition\(rootPx\.x,rootPx\.y\);[\s\S]*?if\(ctxMenu\.style\.display==='block'\)\{[\s\S]*?clampCtxMenu\(\);[\s\S]*?if\(persistOffset\)syncCtxMenuOffsetToAnchor\(\);[\s\S]*?\}[\s\S]*?return true;[\s\S]*?\}/);
