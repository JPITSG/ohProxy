...
// Stale-tile routes: a tile past its max age was refreshed successfully, or
// the refresh failed and the old copy was served instead.
const ROUTE_REFRESHED = 'refreshed';
const ROUTE_STALE = 'stale';
...
function createTileCache(options) {
	...
	// Number or function returning a number (so live config reloads apply).
	// Cached tiles older than this are refreshed on access; a failed refresh
	// serves the old copy. Omit for never-expiring behaviour.
	const maxAgeMsOption = opts.maxAgeMs;
	// Optional extra staleness test (coords, fileMtimeMs) => boolean, ORed
	// with the age check - e.g. "the vector data this render came from is
	// newer than the render".
	const staleIf = typeof opts.staleIf === 'function' ? opts.staleIf : null;
	...
	function currentMaxAgeMs() {
		const value = typeof maxAgeMsOption === 'function' ? maxAgeMsOption() : maxAgeMsOption;
	...
	// Returns { body, stale } for a usable cached tile, else null. A tile is
	// stale once its file is older than the configured max age.
	function readTile(coords) {
		...
		try {
			...
			try {
				...
				stale = (Date.now() - stat.mtimeMs) > currentMaxAgeMs();
				if (!stale && staleIf) stale = staleIf(coords, stat.mtimeMs) === true;
			} catch {}
			return { body, stale };
		} catch (err) {
	...
	// staleBody, when present, is the expired-but-usable copy: any failure to
	// obtain a fresh tile falls back to serving it rather than erroring.
	async function fetchAndStore(coords, staleBody) {
		const key = tileKey(coords);
		const staleResult = (detail) => ({
			route: ROUTE_STALE,
			status: 200,
			body: staleBody,
			contentType: TILE_CONTENT_TYPE,
			detail: `refresh failed (${detail}), serving stale copy`,
		});
		let result;
		try {
			// coords is passed alongside the URL so alternative miss-path
			// implementations (e.g. the local vector renderer) can ignore the
			// upstream URL entirely.
			result = await fetchTile(upstreamTileUrl(coords), coords);
		} catch (err) {
			log(`[MapTiles] ${key} upstream fetch failed: ${err.message || err}`);
			if (staleBody) return staleResult('upstream fetch failed');
		...
		if (!result || !result.ok) {
			...
			if (staleBody) return staleResult(`upstream status ${status}`);
		...
		if (!isPngBuffer(result.body)) {
			...
			if (staleBody) return staleResult('upstream body not a PNG');
		...
		return {
			route: staleBody ? ROUTE_REFRESHED : ROUTE_MISS,
			status: 200,
			body: result.body,
			contentType: TILE_CONTENT_TYPE,
			detail: (staleBody ? 'refreshed from upstream' : 'fetched upstream and cached') + (stored ? '' : ', cache write failed'),
		};
	}
	...
	async function get(coords) {
		...
		if (cached && !cached.stale) {
			return { route: ROUTE_HIT, status: 200, body: cached.body, contentType: TILE_CONTENT_TYPE, detail: 'served from local cache' };
		}
		...
		if (pending) {
			...
			const shared = result.route === ROUTE_MISS || result.route === ROUTE_REFRESHED;
			return {
				...result,
				route: shared ? ROUTE_COALESCED : result.route,
				detail: shared ? 'joined in-flight upstream fetch' : result.detail,
			};
		}

		const task = fetchAndStore(coords, cached ? cached.body : null);
